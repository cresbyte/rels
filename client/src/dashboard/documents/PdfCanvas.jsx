...
export default function PdfCanvas({ fileUrl, documentId, documentType = 'self-sign' }) {
  ...
  // Separate useEffect for widgets
  useEffect(() => {
    fetchWidgets();
  }, [fetchWidgets]);

  // Separate useEffect for document loading
  useEffect(() => {
    if (documentId) {
      loadDocument();
    }
  ...
  const saveDocument = useCallback(async () => {
    ...
    // Use functional update to get current fields
    return new Promise((resolve, reject) => {
      setFields((currentFields) => {
        (async () => {
          try {
            console.log("Starting save process with fields:", currentFields);
            ...
            // Update fields state with new IDs
        ...
        // Return current fields unchanged for now
  ...
  const handleSaveAndDownload = useCallback(async () => {
    try {
      ...
      // First save the document and get updated fields
      await saveDocument();
      
      // Get current fields state for PDF generation
      setFields((currentFields) => {
        // Merge and download using current fields
        (async () => {
          try {
            const pdfBytes = await mergeFieldsIntoPDF(fileUrl, currentFields);
  ...
  }, [api, documentId, selectedContacts, saveDocument]);
